...
In the pase, the Poisson-Boltzmann equation proved to be a good model at high values of surface charge,\cite{BuVakninTravesset2006} that exceeded the surface charge density we use in this work.
...
For example, the absolute value of the nondimensional potential on the molecular surface of an isolated \ig 2 immersed in a solvent with 37mM of salt, averages 1.5, and exceeds $1$ in over $55\%$ of its area.
However, the solvation energy computed with the linear Poisson-Boltzmann equation differs by only $0.02\%$ compared its value obtained with the full Poisson-Boltzmann equation.
If we add a charged surface with $\sigma=0.1$C/m$^2$, and place the \ig 2 as in Fig. \ref{fig:1IGT_2D_sig020_kappa003125} (the preferred orientation in this case), the nondimensional electrostatic potential on the molecular surface has an average absolute value of 1.3, and is above $1$ in $51\%$ of its area, hence, we expect the linearized Poisson-Boltzmann equation to yield a good approximation of the solvation energy.
In the present simulations, the probability distribution was driven by the solvation energy over the surface energy.
...
\begin{figure}[h] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=0.4\textwidth]{Figure16.pdf} 
